...
(function(){
  ...
  function cleanProjectMeta(){
    document.querySelectorAll('.project-card-meta').forEach(meta=>{
      ...
      if(subjects){subjects.style.whiteSpace='nowrap';subjects.style.overflow='hidden';subjects.style.textOverflow='ellipsis';}
    });
  }
  function watchProjectCards(){
    cleanProjectMeta();
    document.querySelectorAll('#featured-projects,#additional-projects,[data-project-list]').forEach(root=>{
      if(root.dataset.metaObserver)return;
      root.dataset.metaObserver='true';
      new MutationObserver(cleanProjectMeta).observe(root,{childList:true,subtree:true});
    });
  }
  function init(){
    loadOverrides();
    ...
    watchProjectCards();
    // Load the editorial layer independently of data.json so a slow data request
    // can never prevent the visual layer from initializing.
    loadEditorial();
  }
