...
(function(){
  function install(){
    ...
    style.textContent=`
      /* Use the clean original headshot rather than the background-removed cutout. */
      .profile-photo{content:url('/assets/img/headshot.jpg') !important; object-fit:cover;}

      /* Keep the two floating actions stacked consistently on every page. The contact
         dialog intentionally sits above Ask Erik when opened. */
      ...
      #contact-panel{z-index:60 !important;}
      ...
      /* Contact overlays the Ask Erik control while its panel is open. */
      #contact-panel[aria-hidden="false"]{z-index:70 !important;}

      /* Make editorial annotations hug their actual text boxes rather than a full grid cell. */
      ...
      /* Slightly slower typewriter: deliberate editorial entrance rather than loading-state feel. */
      .type-text{white-space:pre-wrap;}

      /* More editorial masthead hierarchy without changing the selected forest image. */
      ...
      /* Give At a glance the feel of an editorial deck rather than a résumé block. */
      ...
      /* Subject pills: quiet inline metadata, closer to highlighted code-like labels. */
      ...
      /* Disclaimer is intentionally quiet and subordinate to the answer. */
